...
- [x] **OPC UA** (`asyncua`).
- [x] **S7** (`python-snap7`).
- [ ] **EtherNet/IP**: `cpppo` проверен и отбракован — его собственная пара
      сервер+клиент отдаёт `None`/status 8 (проверено на Python 3.12 и 3.14), а
      программного API сервера для привязки тегов нет. Нужен отдельный минимальный
      CIP-сервер (explicit messaging) или другая библиотека; не входит в поставку.
