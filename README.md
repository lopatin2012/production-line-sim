...
Откройте `http://127.0.0.1:9200/` — панель линии: карточки принтеров, тумблеры
неисправностей (нет бумаги/риббона, головка поднята, пауза, износ головки и др.),
кнопки «Напечатать N» и «Сброс». Состояние обновляется автоматически.

## Control API

По умолчанию `http://127.0.0.1:9200`. `GET /` и `GET /ui` отдают веб-интерфейс,
`GET /health` — JSON.
